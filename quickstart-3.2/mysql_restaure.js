            ...
            connectionfile.connection.query(sql, function (err, results) {
                ...
                if (results[0].cron_date === null) {
                    ...
                    try {
                        ...
                        exec('mysql dataviz_fish_uk < ~/node-extract_sql-insert-p-clean/bdd_mysql/backup/'+ today +'.dump.sql', (error, stdout, stderr) => {    // *** Note: utilisation (plus sécurisée) du fichier '.my.cnf' de '/Home' (= données utilisateur et mot de passe)
